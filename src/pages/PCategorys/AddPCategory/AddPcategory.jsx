...
function AddPcategory() {
            ...
            Services.PCategoryById(
                ...
                .then((response) => {
                    ...
                    if (response.Status === 1) {
                        setCategoryDetails(response.ProfessionCategory);
                        reset({
                            Profession_CategoryName: response.ProfessionCategory.Profession_CategoryName,
    ...
    const onSubmit = (data) => {
        ...
        if (edit) {
            ...
            Services.AddPCategory("POST", JSON.stringify(data), token)
                .then((response) => {
                    if (response.Status === 1) {
                        Navigate(-1);
    ...
    return (
        <Layout Active={"Profession Category"}>
            <Helmet>
                <meta charSet="utf-8" />
                <title>{edit ? "Update Profession" : "Add Profession"} Category</title>
            </Helmet>
            {mainLoad && <MainLoader />}
            <div className={TutorClasses["Container"]}>
                ...
                <div className={TutorClasses["wrapper"]}>
                    <h3>{!edit ? "Add Profession Category" : "Update Profession Category"}</h3>
                    <form onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <div className={TutorClasses["form-control"]}>
                            ...
                            <input
                                {...register("Profession_CategoryName", {
                                    required: "this field is required",
                                    validate: NullCheck,
                                })}
                            />
                            {errors.Profession_CategoryName && (
                                <span>{errors.Profession_CategoryName.message}</span>
                            )}
                        </div>
