...
function Weights() {
    ...
    const getAllCategories = () => {
        Services.Weights("GET", null, token)
            .then((response) => {
    ...
    const editHandler = (e) => {
        ...
        let weight = data.find(w => w.WeightID == id);
        console.log(weight);
        Navigate(`/weight/Update?id=${id}`, { state: weight });
    };
    ...
    const deleteHandler = async (e) => {
        ...
        let id = e.currentTarget.getAttribute("id");
        try {
            const res = await Services.DeleteWeight("PUT", null, token, id)
            if (res.Status === 1) {
                SuccessSwal("weight Deleted", res.Message);
                getAllCategories(1);
    ...
    return (
        <>
            {loading ? (
                <SkeletonLoad
                    TableHead={["Weight", "Status", "Actions"]}
                />
            ) : data.length > 0 ? (
