...
function AddCity() {
  ...
  const onSubmit = (data) => {
    if (edit) {
      let body = data;
      Services.EditCity("PUT", JSON.stringify(body), token, params.get("id"))
        .then((response) => {
          if (response.Status === 1) {
            SuccessSwal("City Updated", response.Message);
            Navigate(-1);
      ...
      Services.AddCity("POST", JSON.stringify(data), token)
        .then((response) => {
          if (response.Status === 1) {
            Navigate(-1);
            SuccessSwal("City Added", response.Message);
          } else if (response.Status === 0) {
            setLoading(false);
            warningSwal("Warning", response.Message);
          }
  ...
  return (
    <Layout Active={"Cities"}>
      <Helmet>
        <meta charSet="utf-8" />
        <title>{edit ? "Update" : "Add"} City</title>
      </Helmet>
      {mainLoad && <MainLoader />}
      <div className={TutorClasses["Container"]}>
        ...
        <div className={TutorClasses["wrapper"]}>
          <h3>{!edit ? "Add City" : "Update City"}</h3>
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className={TutorClasses["form-control"]}>
              <label htmlFor="Priority">
                Priority <span className="important">*</span>
              </label>
              <input
                {...register("Priority", {
                  required: "this field is required",
